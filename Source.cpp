...
word secondkey[4];           //Second key which will be generated from array of keys
 ...
 */
word SubWord(word sw)                          //by reference
{
	word temp;
...
void KeyExpansion(byte key[4 * Nk], word w[4 * (Nr + 1)])
{
	...
	while (i < 4 * (Nr + 1))
	{
		...
		if (i > 4)
		{
			for (int k = 0; k < 4; k++)
			{
				keyArray[keyIndex][k] = (byte(w[i][k])).to_ulong();
			}
			if (i % 4 == 0) {
				ShiftRows(keyArray[keyIndex]);
				MixColumns(keyArray[keyIndex]);
				for (int k = 0; k < 4; k++)
				{
					w[restorIndex] = Word(keyArray[keyIndex][4 * k], keyArray[keyIndex][4 * k + 1], keyArray[keyIndex][4 * k + 2], keyArray[keyIndex][4 * k + 3]);
					restorIndex++;
				}
				keyIndex++;
			}
...
int main()
{
	...
	char* original_text = new char[ConstSize];
	...
	if (infile.fail())
	{
		cout << "your file doesn't work " << endl;
	}
	else
	{
		while (!infile.eof())
		{
			infile >> original_text[arr_size];
			//cout << original_text[arr_size];
			arr_size++;
		}
		infile.close();
	}

	long int textRounds = arr_size / 16;
	long int size = (arr_size / 16 * 16) + (arr_size % 16) ? 16 : 0;
	byte* plain = new byte[size];
	cout << endl << endl << "The key is:";
	for (int i = 0; i < 16; ++i)
	...
	cout << "------------------------ cost time KeyExpansoin---------------------- " << endl;
	cout << "value is :" << elapsed_time << endl;
	cout << "------------------------ cost time KeyExpansoin---------------------- " << endl;



	//-----------------------------------------------------------------------//	
	char* encryptedText = new char[ConstSize];
	char* decryptedText = new char[ConstSize];
	...
	long int ie = 0;
	for (i; i < textRounds; i++)
	{

		for (int k = 0; k < 16; k++)
		{
			plain[k] = original_text[k + 16 * i];
		}
		...
		for (int j = 0; j < 16; j++)
		{
			encryptedText[ie] = (char)plain[j].to_ulong();
			ie++;
		}
	...
	cout << "------------------------ cost time Encryption---------------------- " << endl;
	cout << "value is :" << elapsed_time << endl;
	cout << "------------------------ cost time Encryption---------------------- " << endl;
	...
	for (i; i < textRounds; i++)
	{
		...
		for (int j = 0; j < 16; j++)
		{
			//	cout << (char)plain[j].to_ulong();
			decryptedText[id] = (char)plain[j].to_ulong();
			id++;
		}
	...
	cout << "------------------------ cost time Decryption ---------------------- " << endl;
	cout << "value is :" << elapsed_time << endl;
	cout << "------------------------ cost time Decryption---------------------- " << endl;
	...
	//cout << endl << "Plaintext to be encrypted:" << endl << endl;
	//cout << original_text << endl << endl << endl << endl << endl;;

	cout << "Encrypted ciphertext:" << endl << endl;
	//	cout << encryptedText << endl << endl;  	         // breakdown or output is sound, i don't know -_-. when input is --->  jkdftruiooiuyutbv cfderioijnbnvfdertyuinbnvfgdrytuijnm cvdjh

	for (long int i = 0; i < arr_size-1; i++)
	{
		if (i > 0 && i % 16 == 0) cout << endl;
		cout <<bitset<8>(byte(encryptedText[i]))<<' ';
	}


	cout <<endl<<endl<< "Decrypted plaintext:" << endl << endl;
